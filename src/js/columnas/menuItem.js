...
async function wrapperFunc() {
    try {
        let images = await loadCols();
        return images;     // this will be the resolved value of the returned promise
    } catch(e) {
        console.log(e);
...
export default class MenuItem {
    ...
    layout() {
        ...
        wrapperFunc().then(result => {
            this.DOM.revealImage.style.backgroundImage = `url(${result[this.inMenuPosition][1]})`;
            // got final result
        }).catch(err => {
            // got error
